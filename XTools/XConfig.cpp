...
XConfig::XConfig()
{
	...
	cfgFile = strcat(temp,"\\XTools.ini");
	...
	enableAim          = 0;//inireader->GetValueI("KeySettings", "enableAim");
	menuUp             = 0;//inireader->GetValueI("KeySettings", "menuUp");
	menuDown           = 0;//inireader->GetValueI("KeySettings", "menuDown");
	menuPlus           = 0;//inireader->GetValueI("KeySettings", "menuPlus");
	menuMinus          = 0;//inireader->GetValueI("KeySettings", "menuMinus");
	priFire			   = 0;//inireader->GetValueI("KeySettings", "priFire");
	altFire            = 0;//inireader->GetValueI("KeySettings", "altFire");
	jump			   = inireader->GetValueI("KeySettings", "jump");
	mouseSpeed		   = 0;//inireader->GetValueF("KeySettings" , "mouseSpeed");
	enableDodge		   = inireader->GetValueI("KeySettings", "enableDodge");
	enableHeadShot	   = 0;//inireader->GetValueI("KeySettings", "enableHeadShot");
	...
	autoSwap		   = 0;//inireader->GetValueI("KeySettings", "autoSwap");

	stealthMode				= 0;//inireader->GetValueI("XBotStatus", "stealthMode");
	autoAimEnabled			= 0;//inireader->GetValueI("XBotStatus", "autoAimEnabled");
	radarEnabled			= 0;//inireader->GetValueI("XBotStatus", "radarEnabled");
	imodeType  				= 0;//inireader->GetValueI("XBotStatus", "imodeType");
	headShotEnabled			= 0;//inireader->GetValueI("XBotStatus", "headShotEnabled");
	pingCompValue			= 0;//inireader->GetValueI("XBotStatus", "pingCompValue");
	dodgeEnabled			= inireader->GetValueI("XToolsStatus", "dodgeEnabled");
...
void XConfig::buildDefault()
{
	...
	inireader->SetValueI("XToolsStatus", "dodgeEnabled"			, 3);
}
						 ...
						 bool headshot, int pingComp, int dodge)
{
	...
	inireader->SetValueI("XToolsStatus", "dodgeEnabled"			, true);
