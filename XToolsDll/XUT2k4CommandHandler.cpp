...
void XUT2k4CommandHandler::processParsedCommand(char* &result)
{
	...
	if(funcName == "backgroundmode")
		result = onBkgrnd();
		
	else if(funcName == "setalpha")
		result = onSetAlpha();
/*
	else if(funcName == "setautoblastcomborange")
...
/*	
	...
	*/
	...
	else if(funcName == "team")
		result = onTeam();

	else if(funcName == "enableradar")
		result = onEnableRadar();
...
void XUT2k4CommandHandler::buildCommandList()
{
	
	static string Acommand[] = {/*string("addAutoSwapWeaponCombo"),*/string("addToStartup"),string("")};
	static string Bcommand[] = {/*string("backgroundMode"),*/string("")};
	static string Ccommand[] = {/*string("clearAutoSwapWeaponComboList"),*/string("clrScr"), /*string("connect"),*/string("")};
	static string Dcommand[] = {/*string("disconnect"),*/string("")};
	static string Ecommand[] = {string("enableTimer"),string("enableTimerSound"),/*string("enableRadar"),*/string("")};
	static string Rcommand[] = {/*string("remoteKill"), */string("")};
	static string Scommand[] = {/*string("setWeaponRange"), string("setImodeType"), string("setAimSpeed"), /*string("setUnlockStrenght"),string("setPlayerSpeedMode"),
								string("setXHairSize"),string("setAutoBlastComboRange"), string("say"),*/string("setAlpha"),string("")};
	...
	static string Tcommand[] = {string("team"),string("")};
...
void XUT2k4CommandHandler::onChatMessage()
{
		...
		xbot->netHandler->sendChatMessage(rawCommand.c_str());
		
		rawCommand.insert(0,"<You Wrote>: ");
		xbot->console->addHistoryLine(rawCommand.c_str(),0xFF00FFFF);
}

char* XUT2k4CommandHandler::onEnableRadar()
{
	if(XBot::getXBot()->radarSwitch == true)
	{
		XBot::getXBot()->radarSwitch = false;
		return "radar off";
	}
	else
	{
		XBot::getXBot()->radarSwitch = true;
		return "radar on";
	}
}
...
char* XUT2k4CommandHandler::onConnect()
{
	...
	if(nFuncArgs == 0)
	{
		XBot::getXBot()->netHandler->connect();
		return result = 0;
	}
	else
		return result = "Bad Function Arguments";
...
char* XUT2k4CommandHandler::onDisconnect()
{
	...
	if(nFuncArgs == 0)
	{
		XBot::getXBot()->netHandler->disconnect();
		return result = 0;
	}
	else
		return result = "Bad Function Arguments";
...
char* XUT2k4CommandHandler::onGetPlayerList()
{
	...
	if(nFuncArgs == 0)
	{
		XBot::getXBot()->netHandler->printClientList();
		return result = 0;
...
char* XUT2k4CommandHandler::onTeam()
{
	char* result;
	...
	if(nFuncArgs == 1)
	{
		...
		if(funcArgs[0]=="red")
		{	
			XBot::getXBot()->playerTeam = 30;
			return result = 0;
		}
		else if(funcArgs[0]=="blue")
		{
			XBot::getXBot()->playerTeam = 20;			
			return result = 0;
		}
		else if(funcArgs[0]=="dm")
		{
			XBot::getXBot()->playerTeam = 10;			
			return result = 0;
		}
	}
