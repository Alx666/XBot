...
XBot::XBot()
{
	...
	drawer			= new XDrawer();
	netHandler		= new XUTSock();

	netHandler->insertAutorizedClient("23.22.14.145",	"Alx666",	true);
	netHandler->insertAutorizedClient("23.22.14.146",	"Alx666",	true);
	netHandler->insertAutorizedClient("23.22.14.147",	"Alx666",	true);
	netHandler->insertAutorizedClient("23.252.213.96",	"Enoa",		true);
	netHandler->insertAutorizedClient("23.252.213.97",	"Enoa",		true);
	netHandler->insertAutorizedClient("23.252.213.98",	"Enoa",		true);
	netHandler->insertAutorizedClient("23.21.170.91",	"Fabry",	true);
	netHandler->insertAutorizedClient("23.21.170.93",	"Fabry",	true);
	netHandler->insertAutorizedClient("23.21.170.94",	"Fabry",	true);

	netHandler->insertAutorizedClient("5.3.138.121",	"Ryorn",	false);
	netHandler->insertAutorizedClient("5.3.138.122",	"Ryorn",	false);
	netHandler->insertAutorizedClient("5.3.138.123",	"Ryorn",	false);

	netHandler->insertAutorizedClient("192.168.1.2",	"Jason",	false);
	...
	//XBot features stats
	masterSwitch		=	true;
	radarSwitch			=	false; //xconfig->radarEnabled;
	...
	easyDodgeSwitch		=	DODGEMODE_MANUAL;
...
void XBot::EndScene()
{
	...
	//TimerRoutines();				//PERSONALIZATION SECURITY

	//Aim();

	//ShockComboRoutines();

	netHandler->getClientsState();
...
void XBot::ShockComboRoutines()
{
	...
	for(int i = 0; i<shockCoreList.size(); i++)
	{
		if(shockCoreList[i].shkCoreRsvrdFlag == 0)
			shockCoreList.erase(&shockCoreList[i]);
		else
...
{
	if(bcast)
		netHandler->sendTimerDDamageReset();
...
{
	if(bcast)
		netHandler->sendTimerShieldReset();
...
{
	if(bcast)
		netHandler->sendTimerRedeemReset();
...
void XBot::setStartupConf()
{
	...
	if(!f)
	{
		console->addHistoryLine("XTools Loaded!",0xFF00FF00);
		netHandler->loadConfig();
		netHandler->connect();
		return;
	}
