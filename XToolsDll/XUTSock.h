...
class XUTSock : public XSock
{
	...
	private:
		...
		char* getPlayerName(const in_addr* address);
		...
		void processGetCdkeys(const DataPacket* data, const in_addr* sender);
		void processSaveCdkeys(const DataPacketEx* data, const in_addr* sender);
