...
{	
	//Crash Reason: 01
	//_strupr((char*)extension);

	//Fix
	string sStr = string(extension);
	string sUpper = StringToUpper(sStr);
	const char* pResult = sUpper.c_str();

	//iteriamo la mappa dei file, se troviamo il file cercato, diamo la sua posizione all'interno del file
	list<XFF_FILEDESCRIPTOR>::iterator listIt = xffMap[pResult].begin();

	while(listIt!=xffMap[pResult].end())
	{
