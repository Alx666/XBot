...
XMenu::XMenu(char* launcherPath, const D3DVIEWPORT8* viewport)
{
	...
	xPos = (float)1600 - showingArea;
	yPos = (float)1200 - altitude;
	//xPos = viewport->Width - showingArea;
	//yPos = viewport->Height - altitude;
...
void XMenu::loadTexture()
{
	...
	fsize = xbot->xffReader->GetFile("UT2k4_autoswap", "bmp", &data);
	D3DXCreateTextureFromFileInMemoryEx(d3dDevice,data,fsize, 32, 32, 0, 0, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, D3DX_FILTER_LINEAR, D3DX_FILTER_LINEAR, 0xFF000000, NULL, NULL, &counter);
	delete[] data;
...
//**************************************************************************************************************
	...
	if(!isOut)
	{
		...
		if(XBot::getXBot()->counterShockEnabled)
			spriteDrawer->Draw(counter, NULL, NULL, NULL, 0, &D3DXVECTOR2(xPos + 30, yPos+245), menuColor);
