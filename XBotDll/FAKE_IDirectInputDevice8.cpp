	...
	if(*pdwInOut == 0)		//TODO: (DISCORSO DELLA SENSIBILITA' DEL MOUSE)
	{
			...
			{				
				*pdwInOut = 1;
				...
				rgdod->dwData = xbot->moveMouseY;
				xbot->moveMouseY = 0;
			}

			if(xbot->inputHandler->releaseAltFireShock && GetTickCount() - xbot->inputHandler->releaseAltFireShockDelay >100)
			{
				rgdod->dwOfs = xbot->keyAltFireDInput;
				rgdod->dwData = 0;
				*pdwInOut = 1;
				xbot->inputHandler->releaseAltFireShock =false;
			}
			else if(xbot->inputHandler->rocket2ndFireRelease && GetTickCount() - xbot->inputHandler->release2ndRocketFireDelay > 1700)
			{ 
				if(xbot->tPrimary->isInRange)
					xbot->setAutoAim(true);
				
				xbot->inputHandler->rocket2ndFireRelease = false;
			}

		}
	else if(xbot->gameStarted && xbot->masterSwitch)
	{
		
		if(rgdod->dwOfs == xbot->keyPrimFire) 
		{
			if(rgdod->dwData != 0)
				xbot->inputHandler->onMousePrimDown(pdwInOut);
			else
				xbot->inputHandler->onMousePrimUp(pdwInOut);
		}
		else if(rgdod->dwOfs == xbot->keyAltFireDInput)		//TODO IMPORTANTE gestione autofire fuoco secondario
		{
			if(rgdod->dwData != 0)
				xbot->inputHandler->onMouseSecDown(pdwInOut);
			else
				xbot->inputHandler->onSecFireUp((UINT*)pdwInOut);
		}
		else if(rgdod->dwOfs == xbot->keyJumpDInput)
		{
			xbot->inputHandler->onJumpDown((UINT*)pdwInOut);
		}
		...
		switch (rgdod->dwOfs)
		{
			...
			case DIMOFS_BUTTON7:
				break;
			case DIMOFS_X:
				if (abs((int)rgdod->dwData)>xbot->unlockStr)
				{
