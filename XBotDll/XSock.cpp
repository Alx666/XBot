...
void XSock::connect()
{
	...
	for(UINT i = 0; i < ipIndex; i++)
	{
...
#endif
		{
			...
			//notifica tentativi in corso
			//char *ClientIp = inet_ntoa(ipList[i]->addressTCP.sin_addr);
			//logMessage("XSocket: connecting to %s", ClientIp);
...
//**************************************************
void XSock::sendTcpMessage(const DataPacket* data, const in_addr* sender)
{
	for(UINT i = 0; i<clientNum; i++)
	{
		if(!sender)
		...
		{
			if(client[i]->addressTCP.sin_addr.S_un.S_addr == sender->S_un.S_addr)
			{
				client[i]->sendTcpMessage(data);
				break;
			}
...
//Insert an IP into the autorized clients vector
//**********************************************
void XSock::insertAutorizedClient(char* ip, char* name, bool isMaster)
{
	SecureClient sc;
	sc.address.S_un.S_addr = inet_addr(ip);
	sc.isMaster = isMaster;
	strcpy(sc.name, name);

	autorizedClients.push_back(sc);
}

//check if the current XSocket instance is running on an autorized IP address
//***************************************************************************
bool XSock::isAutorized(SecureClient* owner)
{
	SecureClientVector::iterator it;
	bool found = false;

	for(it = autorizedClients.begin(); it != autorizedClients.end() && !found; it++)
	{
		if(it->address.S_un.S_addr == this->id)
		{
			found = true;
			if(owner)
				memcpy(owner, it._Myptr, sizeof(SecureClient));
		}
	}
...
//check if the current XSocket instance has master rights
//***************************************************************************
bool XSock::isMaster(SecureClient* owner)
{
	SecureClientVector::iterator it;
	bool found = false;
	...
	for(it = autorizedClients.begin(); it != autorizedClients.end() && !found; it++)
	{
		if(it->address.S_un.S_addr == this->id && it->isMaster)
		{
			found = true;
			if(owner)
				memcpy(owner, it._Myptr, sizeof(SecureClient));
...
void XSock::startup()
{
	...
	if(isAutorized())
	{
		...
		if(isMaster())
		{
			logMessage("XSocket: startup completed, Ip:%s",  inet_ntoa(serverAddrTCP.sin_addr));
			logMessage("Admin access granted!");
		}
		else
		{
			logMessage("XSocket: startup completed, Ip:%s",  inet_ntoa(serverAddrTCP.sin_addr));
			SecureClient sc;
			isAutorized(&sc);
			logMessage("licensed to %s", sc.name);
		}
	...
	{
		logMessage("XSocket: Security violation! Shutdown in %i", shutdownTime);
	}
...
void XSock::securityCheck()
{
	if(!ipCheckPassed) //security check still not passed
	{
		if(started)	   //if everything went normal	
		{
			if(GetTickCount() - shutdownTimeElapsed >= 1000) //every second
			{
				shutdownTimeElapsed = GetTickCount();
				logMessage("shutdown in %i", shutdownTime);
				shutdownTime--;
		...
		else		   //if the network subsys cannot start
		{
			if(GetTickCount() - shutdownTimeElapsed >= 1000) //every second
			{
				shutdownTimeElapsed = GetTickCount();
				logMessage("AUTORIZING TIME");
				shutdownTime--;
